...
df = pd.read_excel('output/teste_iss_corrigido.xlsx', sheet_name='Dados_NFSe')

# Verificar se o ISS próprio está sendo calculado corretamente
df['ISS Próprio Calculado'] = df['Base de Cálculo (R$)'] * df['Alíquota (%)'] / 100
df['Diferença ISS Próprio'] = df['ISS Próprio (R$)'] - df['ISS Próprio Calculado']

# Verificar se há diferenças no ISS próprio
diff_iss_proprio = df[abs(df['Diferença ISS Próprio']) > 0.01]
if not diff_iss_proprio.empty:
    print(f"\nEncontradas {len(diff_iss_proprio)} linhas com diferença no ISS próprio")
    print(diff_iss_proprio[['Número da Nota', 'Base de Cálculo (R$)', 'Alíquota (%)', 'ISS Próprio (R$)', 'ISS Próprio Calculado', 'Diferença ISS Próprio']].head())
else:
    print("\nNenhuma diferença encontrada no ISS próprio na aba Dados_NFSe.")
...
wb = load_workbook('output/teste_iss_corrigido.xlsx')
ws = wb['Resumo_NFSe']
...
for row in range(1, ws.max_row + 1):
    ...
    if cell_value and isinstance(cell_value, str) and cell_value.upper() == "CÓDIGO DO SERVIÇO":
        tabelas.append(("Resumo por Código do Serviço", row))
    elif cell_value and isinstance(cell_value, str) and "SITUAÇÃO" in cell_value.upper():
        tabelas.append(("Resumo por Situação", row))
    elif cell_value and isinstance(cell_value, str) and "COMPETÊNCIA" in cell_value.upper():
